...
ENTRYPOINT ["/bin/sh", "-c", "if [ \"$TRANSPORT_TYPE\" = \"stream_http\" ]; then /app/.venv/bin/uv run db2-mcp-server --transport stream-http; else /app/.venv/bin/uv run db2-mcp-server; fi"]
