...
def test_list_tables_success_no_filter(mock_ibm_db):
  ...
  mock_ibm_db.prepare.assert_called_once_with(
    mock_conn, "SELECT TABNAME FROM SYSCAT.TABLES WHERE TYPE = 'T' ORDER BY TABNAME"
  )
...
def test_list_tables_prepare_error(mock_ibm_db):
  ...
  # Act & Assert
  with pytest.raises(
    RuntimeError, match="Failed to prepare SQL statement: Syntax error"
  ):
    list_tables_logic(args)
...
def test_list_tables_execute_error(mock_ibm_db):
  ...
  # Act & Assert
  with pytest.raises(
    RuntimeError, match="Failed to execute SQL statement: Table not found"
  ):
    list_tables_logic(args)
